    ...
    setTimeout(() => {
      const scoredPets = allPets.map((pet) => {
        ...
        if (preferences.petType === "Any" || preferences.petType === pet.type) {
          ...
          // If type doesn't match, this is a deal-breaker
          return { pet, score: 0 }
        }

        // Size matching
        if (preferences.size === "Any" || preferences.size.toLowerCase() === (pet.size || "medium").toLowerCase()) {
          score += 20
        }
        ...
        if (preferences.age === "Any") {
          ...
          (preferences.age === "Baby" &&
            (petAge.includes("baby") || petAge.includes("puppy") || petAge.includes("kitten"))) ||
          (preferences.age === "Young" && (petAge.includes("young") || petAge.includes("adolescent"))) ||
          (preferences.age === "Adult" && (petAge.includes("adult") || petAge.includes("mature"))) ||
          (preferences.age === "Senior" &&
            (petAge.includes("senior") || petAge.includes("older") || petAge.includes("elderly")))
        ) {
          score += 15
        }

        // Energy level matching (based on breed or description keywords)
        const petDescription = (pet.description || "").toLowerCase()
        if (preferences.energyLevel === "Any") {
          score += 15
        } else if (
          (preferences.energyLevel === "Low" &&
            (petDescription.includes("calm") ||
              petDescription.includes("relaxed") ||
              petDescription.includes("lazy"))) ||
          (preferences.energyLevel === "Medium" &&
            (petDescription.includes("playful") || petDescription.includes("friendly"))) ||
          (preferences.energyLevel === "High" &&
            (petDescription.includes("energetic") ||
              petDescription.includes("active") ||
              petDescription.includes("athletic")))
        ) {
          score += 15
        }
        ...
        if (
          ...
          (petDescription.includes("energetic") || petDescription.includes("active"))
        ) {
          score += 10
        } else if (
          preferences.lifestyle === "Moderate" &&
          (petDescription.includes("playful") || petDescription.includes("friendly"))
        ) {
          score += 10
        } else if (
          preferences.lifestyle === "Relaxed" &&
          (petDescription.includes("calm") || petDescription.includes("relaxed") || petDescription.includes("quiet"))
        ) {
          score += 10
        }
        ...
        if (
          preferences.otherPets &&
          (petDescription.includes("good with other pets") || petDescription.includes("gets along with other"))
        ) {
          score += 5
        }
        ...
        if (
          ...
          (petDescription.includes("good with kids") || petDescription.includes("family friendly"))
        ) {
          score += 5
        }
